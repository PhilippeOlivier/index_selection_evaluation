...
struct S_WEB_ORDER_LINEITEM_TBL g_s_web_order_lineitem_web;
struct S_WEB_RETURNS_TBL g_s_web_return;
int nItemIndex_web = -1;
...
mk_detail(int i, int bPrint)
{
      int nTemp;
		...
		mk_s_web_order_lineitem(&g_s_web_order_lineitem_web, i);
      if (bPrint)
         pr_s_web_order_lineitem(&g_s_web_order_lineitem_web);
...
mk_s_web_order(void *pDest, ds_key_t kIndex)
{
   ...
   genrand_integer(&nItemIndex_web, DIST_UNIFORM, 1, (int)getIDCount(ITEM), 0, S_WLIN_ITEM_ID);
	for (i=1; i <= 12; i++)
	{
      mk_detail(i, 1);
   }
